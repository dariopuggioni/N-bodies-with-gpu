...
const float eps    = 1e-1f;    //this has also a physical meaning because at small distances the force is softened due to finite size of bodies and other effects
...
void compute_initial_acceleration(const real* x, const real* y, const real* z,
    ...
    for (int i = 0; i < n_bodies; i++) {
        for (int j = i + 1; j < n_bodies; j++) {
            ...
            dx -= L * round(dx / L);   //boundary periodic conditions
            dy -= L * round(dy / L);
            dz -= L * round(dz / L);
...
int main(void) {
    ...
    real U = 0.0f;
    compute_initial_acceleration(x, y, z, m, ax, ay, az, &U); //argumemts are pointers
    ...
    for (int step = 1; step < steps; step++) {
        ...
        while (1) {
            ...
            real min_r2 = 1e30f;             //for each step find the minimum distance
            int i_min = 0, j_min = 1;
            for (int i = 0; i < n_bodies; i++) {
                for (int j = i + 1; j < n_bodies; j++) {
            ...
            else {dt = dt_required;} //reduce dt and start again the loop from vx_half
        }  //end while
        ...
        for (int i = 0; i < n_bodies; i++) {          //for each pair
            for (int j = i + 1; j < n_bodies; j++) {
                real dx = x_new[j] - x_new[i];
                real dy = y_new[j] - y_new[i];
                real dz = z_new[j] - z_new[i];
                ...
                real f = G * inv_r / r2;  //  G/r^3

                ax_new[i] += f * m[j] * dx;
                ay_new[i] += f * m[j] * dy;
                az_new[i] += f * m[j] * dz;

                ax_new[j] -= f * m[i] * dx;  //minus: the vector points in the opposite direction
                ay_new[j] -= f * m[i] * dy;
                az_new[j] -= f * m[i] * dz;

                U -= G * m[i] * m[j] * inv_r; //pot energy of the pair
            }
        }

        for (int i = 0; i < n_bodies; i++) {
            vx[i] = vx_half[i] + 0.5f * ax_new[i] * dt;  //new vx - we can overwrite the old one
            vy[i] = vy_half[i] + 0.5f * ay_new[i] * dt;
            vz[i] = vz_half[i] + 0.5f * az_new[i] * dt;

            x[i] = FMOD(x_new[i] + L, L); //it does for real numbers n_new % L; +L to avoid negative values
            y[i] = FMOD(y_new[i] + L, L);
            z[i] = FMOD(z_new[i] + L, L);
        ...
        time[step] = time[step - 1] + dt; //update time considering variable dt
        Etot[step] = K + U;         
